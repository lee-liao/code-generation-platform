...
{{"reponseEmail":[{{"language":"en","emailcontent":"email go here", "subject":"", "to":"", "cc":"", "attachments":[]}}, ... other languages}} , the first response should be the original language.
...
"10.可供选择的附件在{attachments},请分析邮件里可能提到的需要的附件，请精确匹配，比如Titan M1-A和Titan M1-AA不是用一个东西，如果有，放到json的attachments字段里，如果没有，则attachments字段保持[],邮件回复内容也相应调整。不要有其他无关内容。
"11.请特别注意，客户信息在customer_开头的字段里，自身信息在sender_开头的字段里，是sender给customer发邮件，不要反过来。不要自动纠错sender给customer里的信息，请务必确保每次都是sender给customer发邮件，且内容也要符合sender给customer发邮件的逻辑。
请严格按以下JSON格式输出分析报告，确保不遗漏任何字段，保持键名完全一致：
## 格式要求
1. 必须使用双引号包裹所有键和字符串值
2. 空值字段用null表示，数值保持整数类型
...
def generate_aiadvice_section(json_data):
    name=json_data['name']
    industryType=json_data['industryType']
    desc=json_data['desc']
    business=json_data['business']
    companyType=json_data['companyType']
...
5. 公司类型：{companyType}
6. 服务内容：{service} 
请根据以上信息，写出AI洞察与推荐，格式如下：
该客户最近有xx需求，根据他们的xx行为和最近xx，预计在xx个月内进行相关领域投资。推荐通过xx方案切入，强调xx，建议xx。
xx部分换成实际的数据。
请严格按以下JSON格式输出分析报告，确保不遗漏任何字段，保持键名完全一致：
## 格式要求
1. 必须使用双引号包裹所有键和字符串值
...
""")
        ...
        logging.error(f"Error generating aiadvice analysis '{name}': {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error generating aiadvice analysis: {str(e)}")

def generate_code_line_section(json_data):
    diff=json_data['diff']
    try:
        response = query_perplexity(f"""\
你是一个代码统计大师，能根据git diff的输出统计出写了多少行代码。
请严格按照以下规则进行统计：
统计代码变更中修改的行数：
1. 找到所有以 "-" 开头的行（不包括空行和仅包含 "-" 的行）。
2. 找到所有以 "+" 开头的行（不包括空行和仅包含 "+" 的行）。
3. 比较这些行的内容，如果删除的行和新增的行内容相同或相似，则认为是修改的行。
4. 统计修改的行数。
我会给你一个或多个git diff的输出，每个git diff算作一个项目。请自动将所有项目的行数加起来赋值给total, add, delete, modify ，total=add + delete + modify 并以JSON格式返回结果，格式如下：
只输出JSON结果，不需要过程。
请严格按以下JSON格式输出分析报告，确保不遗漏任何字段，保持键名完全一致：
## 格式要求
1. 必须使用双引号包裹所有键和字符串值
...
6. 任何情况下都禁止额外的说明，比如根据您的需求或根据您提供的信息之类的说明，只需要返回json，不要有```json和```结尾
## 模板示例
{{"total":"总行数", "add":"新增行数", "delete":"删除行数", "modify":"修改行数"}}
以下是git diff的内容：{diff}
""")
         # 新增多级内容清洗
        return response['choices'][0]['message']['content'].strip()
    except Exception as e:
        logging.error(f"Error generating code line: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error generating code line: {str(e)}")
...
@app.post("/generate-aiadvice")
def generate_aiadvice(json_data):
    data = generate_aiadvice_section(json_data)
    return {"data": data}

@app.post("/generate-code-line")
def generate_code_line(json_data):
    data = generate_code_line_section(json_data)
