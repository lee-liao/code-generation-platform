...
def generate_report_section_by_perplexity(section_name, data):
    try:
        logging.info(f"Generating report section by perplexity: {section_name}")
        prompt = f"Generate a paragraph for the '{section_name}' section of a business report based on the following data: {data}, Please search the internet and provide the best complihensive answer to the questions, please don't repeat yourself, by saiding same thing over and over again."
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are an expert business report writer."},
                {"role": "user", "content": prompt}
            ],
            'max_tokens': 2000,
            'temperature': 0.7
...
def generate_business_report(file_url, session_id, user_id):
    ...
    try:
        ...
        try:
            ...
            redis_client.set(redis_key2, "normal")
        ...
        for section_idx, section in enumerate(template_sections):
            ...
            for subtitle_idx, subtitle in enumerate(section['subtitles']):
                ...
                # 添加子标题内容
                content_paragraph = document.add_paragraph()
                run = content_paragraph.add_run(generate_report_section_by_perplexity(subtitle, data))
                content_font = run.font
                content_font.name = 'SimSun'
                content_font._element.rPr.rFonts.set(qn('w:eastAsia'), 'SimSun')
                content_font.size = Pt(12)
