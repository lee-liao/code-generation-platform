...
8. 这个json出现数字数据(年份除外)的地方，包括单位和百分号请加高亮处理，加上class abc-highlight，最后仍然返回json
...
{{"reponseEmail":[{{"language":"en","emailcontent":"email go here"}}, ... other languages}} , the first response should be the original language.
请严格按以下JSON格式输出分析报告，确保不遗漏任何字段，保持键名完全一致：
## 格式要求
1. 必须使用双引号包裹所有键和字符串值
2. 空值字段用null表示，数值保持整数类型
3. 嵌套结构必须完整保留层级关系
4. 禁止添加注释或额外说明
5. 禁止额外的说明，比如根据您的需求或根据您提供的信息之类的说明，只需要返回json，不要有```json和```结尾
""")
         # 新增多级内容清洗
        return response['choices'][0]['message']['content'].strip()
    except Exception as e:
...
def generate_email(json_data):
    return {"data": generate_email_section(json_data)}
